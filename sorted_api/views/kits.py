...
from ..serializers.kit import KitSerializer, NewKitSerializer
...
class KitsView(APIView):
    def post(self, request):
        ...
        kit = NewKitSerializer(data=request.data)
    
        if not kit.is_valid():
            return Response(kit.errors, status=status.HTTP_400_BAD_REQUEST)
        # print(kit)
        kit.save(owner=request.user)
        newKit = Kit.objects.get(id = kit.data['id'])
        retVal = KitSerializer(newKit)
        # if not retVal.is_valid():
        #     return Response(retVal.errors, status=status.HTTP_400_BAD_REQUEST)
